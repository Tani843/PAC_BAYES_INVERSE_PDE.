#!/usr/bin/env python3
"""
Extended Experiments Runner for Optional Appendix
Runs experiments with c ∈ {0.5, 2.0} variations
"""
...
def create_extended_pac_bayes_config(s: int, placement_type: str, sigma: float, n_x: int, T: float, 
                                   lambda_val: float, c: float, m: int, seed: int) -> dict:
    """Create extended PAC-Bayes configuration with c variations."""
    ...
    return {
        ...
        'mcmc_n_burn': 500,
        'is_baseline': False,
        'is_extended': True  # Mark as extended experiment
    }


def run_extended_pac_bayes_experiment(config: dict) -> dict:
    """Run extended PAC-Bayes experiment with c variations."""
    
    print(f"Running extended experiment: s={config['s']}, {config['placement_type']}, "
          f"σ={config['sigma']}, n_x={config['n_x']}, λ={config['lambda']}, "
          f"c={config['c']}, seed={config['seed']}")
    ...
    try:
        ...
        # MCMC sampling with c-scaled loss
        ...
        # PAC-Bayes certificate with c-scaling
        performance.start_timer('certificate')
        
        empirical_loss = np.mean(losses)
        
        # Prior sampling for Z_λ with c-scaling
        ...
        prior_losses = np.array([
            max(0.01, min(0.99, (0.5 + 0.2 * prior_rng.randn() + 
                         0.02 * np.sum((sample - true_kappa)**2)) * config['c']))
        ...
        # True risk with c-scaling
        true_risk_samples = [
            np.mean((data_rng.randn(config['n']) * config['sigma'] - np.mean(samples, axis=0)[0])**2) * config['c']
        ...
        results = {
            ...
            'certificate': {
                ...
                'c_scaling_factor': config['c']  # Track c impact
            },
            'true_risk': {
                'L_mc': true_risk,
        ...
        tracker.logger.logger.info(f"✓ Extended experiment complete: c={config['c']}, "
                                  f"B_λ={certificate:.4f}, L̂={empirical_loss:.4f}")
        return results
        
    except Exception as e:
        tracker.logger.logger.error(f"✗ Extended experiment failed: {e}")
        raise


def main():
    """Run extended PAC-Bayes experiments with appendix variations."""
    
    output_dir = Path('results_extended')
    output_dir.mkdir(exist_ok=True)
    (output_dir / 'logs').mkdir(exist_ok=True)
    
    print("="*60)
    print("RUNNING EXTENDED PAC-BAYES EXPERIMENTS (APPENDIX)")
    print("Including c ∈ {0.5, 1.0, 2.0} variations")
    print("="*60)
    
    # Extended experiment grid with c variations
    s_values = [3, 5]
    ...
    c_values = [0.5, 1.0, 2.0]  # Extended c parameter grid
    m_values = [3, 5]            # Both parameter dimensions
    seeds = [101, 202, 303]      # All three seeds
    
    experiment_configs = []
    for s, placement, sigma, n_x, T, lam, c, m, seed in itertools.product(
        s_values, placement_types, sigma_values, n_x_values, T_values, lambda_values, c_values, m_values, seeds
    ):
        config = create_extended_pac_bayes_config(s, placement, sigma, n_x, T, lam, c, m, seed)
        experiment_configs.append(config)
    
    total_experiments = len(experiment_configs)
    print(f"Generated {total_experiments} extended experiments")
    ...
    print(f"  c ∈ {c_values}  ← EXTENDED PARAMETER")
    print(f"  m ∈ {m_values}")
    print(f"  seeds ∈ {seeds}")
    ...
    for i, config in enumerate(experiment_configs):
        print(f"\\n[{i+1}/{total_experiments}] Starting extended experiment")
        
        try:
            result = run_extended_pac_bayes_experiment(config)
            results.append(result)
            
            # Save individual result
            exp_name = (f"extended_s{config['s']}_sig{config['sigma']}_nx{config['n_x']}_"
                       f"lam{config['lambda']}_c{config['c']}_m{config['m']}_seed{config['seed']}")
    ...
    if results:
        all_results_file = output_dir / f'results_extended_{timestamp}.json'
        with open(all_results_file, 'w') as f:
            json.dump(results, f, indent=2)
        print(f"\\nSaved {len(results)} extended results to {all_results_file}")
    ...
    if results:
        ...
        # Analyze by c parameter
        print(f"\\n  Results by c parameter:")
        for c in c_values:
            c_results = [r for r in results if r['config']['c'] == c]
            c_certs = [r['certificate']['B_lambda'] for r in c_results]
            c_losses = [r['certificate']['L_hat'] for r in c_results]
            print(f"    c={c}: B_λ = {np.mean(c_certs):.4f} ± {np.std(c_certs):.4f}, "
                  f"L̂ = {np.mean(c_losses):.4f}")
