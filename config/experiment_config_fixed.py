...
class ExperimentConfig:
    ...
    # Number of segments for piecewise constant kappa
    ...
    def get_experiment_grid(self, include_appendix: bool = False) -> List[Dict]:
        ...
        # Determine c values based on appendix flag
        c_values = [self.c_main] if not include_appendix else [self.c_main] + self.c_appendix
        
        # Generate ALL combinations (this generates the complete 1,728 experiment grid)
        for s, placement_type, sigma, n_x, T, lambda_val, c, m, n_t, seed in itertools.product(
            self.s_values,           # [3, 5]
            ['fixed', 'shifted'],    # Both placement types
            self.sigma_values,       # [0.05, 0.10, 0.20]
            self.n_x_values,         # [50, 100]
            self.T_values,           # [0.3, 0.5]
            self.lambda_values,      # [0.5, 1.0, 2.0]
            c_values,               # [1.0] or [1.0, 0.5, 2.0]
            self.m_values,          # [3, 5]
            self.n_t_values,        # [50, 100]
            self.seeds              # [101, 202, 303]
        ):
            config = {
                's': s,
    ...
    def get_baseline_subset(self) -> List[Dict]:
        ...
        all sigma, one T (e.g., 0.5), both m{3,5}.
        """
    ...
    def log_config(self) -> str:
        ...
        config_str += f"Noise levels (sigma): {self.sigma_values}\n"
        config_str += f"Mesh sizes (n_x): {self.n_x_values}\n"
        config_str += f"Time horizons (T): {self.T_values}\n"
        config_str += f"Temperature params (lambda): {self.lambda_values}\n"
        config_str += f"Segments (m): {self.m_values}\n"
        config_str += f"Seeds: {self.seeds}\n"
        config_str += f"Certificate confidence (delta): {self.delta}\n"
        config_str += f"Hoeffding confidence (alpha): {self.alpha}\n"
        config_str += f"Prior sampling budget (M): {self.prior_sampling_budget_M}\n"
        config_str += f"MC repeats (R): {self.mc_repeats_R}\n"
        config_str += f"Kappa bounds: [{self.kappa_min}, {self.kappa_max}]\n"
        config_str += "=" * 50 + "\n"
